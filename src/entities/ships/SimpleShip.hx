...
{
	static public inline var delay : Float = 1.5;

	public function new(p_x : Float, p_y : Float, p_speedX : Float, p_speedY : Float, p_sun : Sun)
	{
		super(p_x, p_y, new Image("gfx/simpleship.png"), p_speedX, p_speedY, p_sun, 1, 1);
	}
	...
	public override function update() : Void 
	{
		...
		if (timer >= delay) {
			...
			if (target != null) {
				HXP.world.add(new Bullet(this, Std.int(target.x), Std.int(target.y)));
			}
		}
	}
}